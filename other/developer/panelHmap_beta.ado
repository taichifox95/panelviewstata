...
program define panelHmap
	version 15.1
	syntax varlist(min=3 max=3) [if] [in] [, XLabel(passthru) YLabel(passthru) * BYTIMING CONTINUOUS CONTROL PREPOST(string)]
	marksample touse
	sum `touse'
	
	args tunit ids treat
...
panelHmap year ccode demo,  title("demo score hmap", size(small))


use turnout.dta, replace

panelHmap year abb policy_edr
panelHmap year abb policy_edr, prepost(off)
panelHmap year nids policy_edr if nids > 8, prepost
